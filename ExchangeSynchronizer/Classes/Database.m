...
-(void)createDatabase
{
	...
	char *tempErrorMsg;
	NSString *createSQL = @"CREATE TABLE IF NOT EXISTS Tasks (ID INTEGER PRIMARY KEY, TYPETASK INT, HEADER VARCHAR(100), BODY VARCHAR(200), DATE VARCHAR(30),LASTUPDATE VARCHAR(30),MAILFROM VARCHAR(50),MAILDATE VARCHAR(30), STATE VARCHAR(15),PKEYXML VARCHAR(100), CHANGEKEY VARCHAR(100), DATEFINISH VARCHAR(50),CHANGED VARCHAR(10), USER VARCHAR(20));";
...
-(NSInteger)insert:(Task*)task:(NSString*)login
{
	if (sqlite3_open([pathToDB UTF8String], &db) == SQLITE_OK) {
		...
		const char* sql = [[NSString stringWithFormat:@"%@%@%@%@%@%@%@%@%@%@%@%@%@", @"INSERT INTO Tasks VALUES (NULL ,?, ?, ?,'",[task dateCreated],@"','','','',?,'",[task pKeyFromXML],@"','",[task changeKey] ,@"','",[task finishDate],@"','",chg,@"','",login,@"');"] UTF8String];
		printf("%s",sql);
